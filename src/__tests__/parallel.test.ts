...
describe('processInParallel', () => {
    ...
    test('respects concurrency limit', async () => {
        ...
        const result = await processInParallel([1, 2, 3, 4, 5, 6], 2, async (x) => {
            currentConcurrent++;
    ...
    test('collects errors without stopping', async () => {
        const result = await processInParallel([1, 2, 3, 4, 5], 2, async (x) => {
    ...
    test('handles multiple errors', async () => {
        const result = await processInParallel([1, 2, 3, 4, 5], 3, async (x) => {
    ...
    test('handles all failures', async () => {
        const result = await processInParallel([1, 2, 3], 2, async () => {
    ...
    test('converts non-Error throws to Error', async () => {
        const result = await processInParallel([1], 1, async () => {
            throw 'string error';
        });
    ...
    test('processes in parallel order (not sequential)', async () => {
        ...
        await processInParallel([1, 2, 3, 4], 4, async () => {
    ...
    test('handles concurrency of 1 (sequential)', async () => {
        ...
        await processInParallel([1, 2, 3], 1, async (x) => {
            order.push(x);
    ...
    test('handles more concurrency than items', async () => {
        const result = await processInParallel([1, 2], 10, async (x) => x * 2);

        expect(result.results.sort((a, b) => a - b)).toEqual([2, 4]);
    });

    test('works with async processors that return different types', async () => {
        const result = await processInParallel(['a', 'bb', 'ccc'], 2, async (s) => s.length);
